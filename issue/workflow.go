...
// and its classification into dispatch buckets. It is loaded from the hub and
// project beans.toml files with built-in defaults.
//
// The three buckets:
//
...
//
// Statuses are plain strings; the vocabulary is data, not an enum.
...
func (w WorkflowConfig) IsValid(s string) bool { return containsState(w.Statuses, s) }

// IsActive reports whether s is a dispatchable status.
func (w WorkflowConfig) IsActive(s string) bool { return containsState(w.Active, s) }

// IsTerminal reports whether s is a terminal status.
func (w WorkflowConfig) IsTerminal(s string) bool { return containsState(w.Terminal, s) }
...
func (w WorkflowConfig) IsHold(s string) bool {
...
func (w WorkflowConfig) DefaultState() string {
	if w.Default == "" {
		return "open"
	}
	return w.Default
...
func (w WorkflowConfig) Validate() error {
	...
	known := make(map[string]bool, len(w.Statuses))
	...
	terminal := make(map[string]bool, len(w.Terminal))
	for _, s := range w.Terminal {
		terminal[s] = true
	}
