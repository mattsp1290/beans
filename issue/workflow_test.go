...
func TestDefaultWorkflowConfigBuckets(t *testing.T) {
	...
	for _, s := range []string{"ready_for_review", "ready_for_validation", "ready_for_merge"} {
		if !w.IsValid(s) {
	...
	for _, s := range []string{"closed", "done"} {
...
func TestWorkflowConfigValidate(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			cfg:     WorkflowConfig{Statuses: []string{"open"}, Default: "closed"},
			wantErr: true,
		},
		{
			name:    "active not in statuses",
			cfg:     WorkflowConfig{Statuses: []string{"open"}, Default: "open", Active: []string{"weird"}},
			wantErr: true,
		},
		{
			name:    "terminal not in statuses",
			cfg:     WorkflowConfig{Statuses: []string{"open"}, Default: "open", Terminal: []string{"weird"}},
			wantErr: true,
		},
		{
		...
		},
		{
			...
			cfg:     WorkflowConfig{Statuses: []string{"open", "open"}, Default: "open"},
			wantErr: true,
		},
		{
			...
			cfg: WorkflowConfig{
				Statuses:    []string{"open", "closed"},
				Default:     "open",
				Transitions: map[string][]string{"open": {"ghost"}},
			},
			wantErr: true,
		},
		{
			...
			cfg: WorkflowConfig{
				Statuses:    []string{"open", "closed"},
				Default:     "open",
				Transitions: map[string][]string{"ghost": {"open"}},
			},
			wantErr: true,
		},
		{
			...
			cfg: WorkflowConfig{
				Statuses: []string{"open", "qa", "closed"},
				Default:  "open",
				Active:   []string{"open"},
				Terminal: []string{"closed"},
			},
